...
<div class="container">
    <div class="row m-0"style="padding-top:30px">
        <div class="col-sm-12"style="text-align: center;">
            <p id="apps-txt" style="font-family:quicksand;font-weight: bold;color: #2EA8D1;font-size: 60px;"> APPLICATION SENT TO MUSICIANS</p>
        </div>
    </div>
    ...
    <table class="table table-striped"style="text-align:center;font-family:quicksand;font-weight:bold">
        ...
        @foreach($sent_from as $sent_from)
            <?php
                ...
                $std = Http::get(env('API_URL','34.87.164.62').'/api/getUserDetails/'.$sent_from['sent_to'])[0]
            ?>
            <tr>
        ...
        <div class="row m-0"style="padding-top:30px">
            <div class="col-sm-12"style="text-align: center;">
                <p id="apps-txt"style="font-family:quicksand;font-weight: bold;color: #2EA8D1;font-size: 60px;"> APPLICATION FROM MUSICIANS</p>
            </div>
        </div>
            <div class="table-responsive">
            ...
            <table class="table table-striped"style="text-align:center;font-family:quicksand;font-weight:bold">
                ...
                <?php
                $std = Http::get(env('API_URL','34.87.164.62').'/api/getUserDetails/'.$sent_to['sent_from'])[0]
            ?>
                    <tr>
                    <th scope="row">{{$loop->iteration}}</th>
                    <td>{{ $std['name'] }}</td>
                    ...
                    <?php if($sent_to['application_status']=="Contacted") {?>
                        <td style="color:#5DC86D">
                            <a target="_blank" href="https://wa.me/{{$std['phone'] }}?text=I'm%20interested%20in%20you%20as%20musician">
                                Phone ({{$std['phone'] }})
                            </a>
                        </td>                    <?php }
