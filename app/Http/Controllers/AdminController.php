...
class AdminController extends Controller
{
    ...
    public function login(Request $request) {
        ...
        ])->get('http://127.0.0.1:8000/api/auth/getAdminToken',[
            "username"  => $username
        ]);
        ...
        if($user['message'] === "Your Credential is wrong"){
            ...
            ])->get('http://127.0.0.1:8000/api/auth/getAllMusicians'); 
            $user = json_decode($response->body(), true);
            // dump($user);
            return view('admin/admin-musicians',compact('user'));
            return view('admin/admin-musicians');
        }
    ...
    public function loadBands(){
       if (Session::get('adminLogin')){
        ...
        ])->get('http://127.0.0.1:8000/api/auth/getAllBands'); 
        $user = json_decode($response->body(), true);
        // dump($user);
